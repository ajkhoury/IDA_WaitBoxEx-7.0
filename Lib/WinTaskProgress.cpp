...
void __cdecl TaskProgress::start( HWND hwnd )
{
    if (hwnd)
    {
...
void __cdecl TaskProgress::setProgress( int progress )
{
    if (tbi)
    {
        if (progress < 0)
...
void __cdecl TaskProgress::setTrackingWindow( HWND hwnd )
{
    try
    {
        if (tbi && hwnd)

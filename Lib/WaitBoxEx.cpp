...
// IDA SDK Qt libs @ (SDK)\lib\x64_win_qt
...
{
    WINDOWINFO winInfo;
    ULONG_PTR dwIdaProcessId = *((ULONG_PTR*)lParam);
    DWORD dwProcessId;
...
{ 
    ULONG_PTR dwIdaProcessId = (ULONG_PTR)GetCurrentProcessId( );
    ...
    if (nCode == HC_ACTION)
    {
        ...
        if (ms->message == WM_SIZE)
        {
            if (prgDlg && (ms->hwnd == (HWND)prgDlg->winId( )))
            {
                ...
                //    process_ui_action( "Hide" );
                //else if (ms->wParam == SIZE_RESTORED)
                //    process_ui_action( "UnHide" );
            }
        }
    }
    ...
    m_hMouseHook( NULL ), m_hWinHook( NULL ), m_hTimerQueue( NULL ), m_hUpdateTimer( NULL )
{
    ...
    QFont fnt( FONT, 10, QFont::Normal, false );
